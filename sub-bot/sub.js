...
async function isUserSubscribed(userId) {
  ...
  if (userSubscriptions.has(userId)) {
    const { isSubscribed, expiry } = userSubscriptions.get(userId);
    if (expiry > Date.now()) {
      return isSubscribed;
    }
  }
  ...
  try {
    ...
    if (results.length > 0) {
      ...
      userSubscriptions.set(userId, { isSubscribed, expiry: expiryDateTime.getTime() }); // تحديث حالة الاشتراك في الذاكرة المؤقتة مع وقت انتهاء

      return isSubscribed;
    } else {
      userSubscriptions.set(userId, { isSubscribed: false, expiry: Date.now() }); // إذا لم يكن هناك سجل للمستخدم
      return false;
    }
  } catch (err) {
